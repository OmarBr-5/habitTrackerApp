...
class TestHabitServiceWith4WeekData:
    ...
    def test_streak_calculation_morning_exercise(self):
        ...
        self.habit_repo.fetch_by_id = Mock(
            return_value=self.habits[0]
        )  # Morning Exercise
        ...
        expected_streak = get_longest_streak_of_a_habit(
            self.habits[0], habit1_completions
        )
    ...
    def test_streak_calculation_read_book(self):
        ...
        expected_streak = get_longest_streak_of_a_habit(
            self.habits[1], habit2_completions
        )
